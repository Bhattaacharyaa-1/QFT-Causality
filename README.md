...
This repository implements the statistical frameworks from *Causality: Statistical Perspectives and Applications* (Berzuini, Dawid, Bernardinelli) using free financial datasets.

## 📚 Methodology & Book Mappings
| Module | Causal Method | Book Chapter | Financial Application | Dataset |
| :--- | :--- | :--- | :--- | :--- |
| **01** | Granger / Sims Causality | Ch 22 | Lead-lag order flow & crypto contagion | G-Research Crypto (Kaggle) |
| **02** | Instrumental Variables (IV) | Ch 4, 18 | Isolating liquidity shocks from returns | yfinance + FRED API |
| **03** | DAGs & Back-Door Criterion | Ch 3, 6 | Feature selection (avoiding collider bias) | Two Sigma / Optiver (Kaggle) |
| **04** | G-Computation / MSMs | Ch 8, 17 | Dynamic position sizing & regime switching | Jane Street (Kaggle) |
| **05** | Mediation Formula | Ch 11, 12 | Factor decomposition (Direct vs Indirect Alpha) | Synthetic / Jane Street |
| **06** | Sensitivity Analysis | Ch 19 | Bounding hidden market states (Dark Pools) | Any Observational Data |

## 🛠️ Setup
```bash
pip install -r requirements.txt
# For Kaggle datasets, ensure you have ~/.kaggle/kaggle.json configured
...
------------------------------------------------
            ...
            ▼                    ▼                    ▼
    ┌───────────────┐   ┌───────────────┐   ┌──────────────────┐
    │   VOLUME      │   │   SPREAD      │   │ REALIZED VOLATILITY│
    └───────┬───────┘   └───────┬───────┘   ────────┬─────────┘
            │                   │                     │
            │                   │                     │
            ▼                   ▼                     ▼
